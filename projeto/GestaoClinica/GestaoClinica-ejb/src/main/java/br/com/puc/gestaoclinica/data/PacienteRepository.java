...
public class PacienteRepository {
    ...
    public List<Paciente> recuperarPorNome(String nome) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Paciente> criteria = cb.createQuery(Paciente.class);
        Root<Paciente> paciente = criteria.from(Paciente.class);
        criteria.select(paciente).where(cb.equal(paciente.get("nome"), nome));
        criteria.select(paciente).where(cb.equal(paciente.get("ativo"), Boolean.TRUE));
